...
{ 
    ...
    visited.insert(pair<state, int>(sta, vertex_count));
    queue.push_back(sta); 
    // standard BFS algorithm 
    while (!queue.empty()) {  
        state u = queue.front();
        int u_index = visited.at(u);
        queue.pop_front(); 
        for (int i = 1; i < 5; i++) { 
            state next_st = is_valid_next_step(u, i);
            if (next_st.player_direction != -1) {
                vertex_count ++;
                visited.insert(pair<state, int>(next_st, vertex_count));
                steps[vertex_count] = i;
                pred[vertex_count] = u_index; 
                queue.push_back(next_st); 
                if (is_finish(next_st))   
                        return;
...
{ 
    ...
    steps[0] = 0;
    BFS(st, pred); 
    int crawl = vertex_count; 
    path.push_back(steps[crawl]); 
    while (pred[crawl] != -1) { 
        path.push_back(steps[pred[crawl]]); 
        crawl = pred[crawl]; 
    } 
    ...
    cout << "\n-----START-----\n"; 
    int step_count = 1;
    for (int i = path.size() - 2; i >= 0; i--)  { // -2: exclude origin.  
